...
async function main() {
    try {
        ...
        const userExists = await wallet.get('bob1');
        if (!userExists) {
            console.log('An identity for the user "bob1" does not exist in the wallet');
        ...
        await gateway.connect(ccp, { wallet, identity: 'bob1', discovery: { enabled: true, asLocalhost: true } });
